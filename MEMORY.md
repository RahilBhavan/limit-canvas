...
## 2026-05-21 — Release-polish pass: L11 closed + pre-existing CI failure fixed

What: Two things, both pushed to `main` (public repo `RahilBhavan/limit-canvas`). (1) **Closed L11** — persisted studio state is now schema-validated. `apps/studio/src/lib/persisted-strategy.ts` stamps the saved localStorage blob with `DSL_VERSION` and, on load, re-parses the persisted `doc` through `strategyDocumentSchema.safeParse`; `loadPersistedState` returns a discriminated `empty | ok | incompatible` result. On `incompatible` the wizard clears the stale blob and shows a dismissible `.notice-banner` naming the saved DSL version — fails loudly instead of silently resetting. README/`1inch-review.md`/`CLAUDE.md` updated to mark L11 resolved (only L5 orderbook + L10 multichain remain open). Also added `.claude` to `biome.json` ignore so local `bun run lint` matches CI. (2) **Fixed a pre-existing CI failure** — the `contracts` job had been red on *every* recent run (not just mine): the "Verify LOP submodule pinned to 4.3.2" step ran `git describe --tags --exact-match HEAD`, but `actions/checkout@v4` checks out the submodule at the pinned commit without fetching its tags, so describe exited 128 and aborted the job *before* `forge build`/`forge test` ever ran. Replaced it with a commit-SHA assertion (`rev-parse HEAD` == `67c56aee…`, the 4.3.2 commit). CI is now fully green — both jobs pass, and the contracts job actually exercises Foundry now.

Why: User goal was to clean up and make the repo postable on X. The red CI badge was a real blocker (and meant contract tests weren't running in CI at all). The L11 fix was the last open item that undercut the "reproducible" pitch.

Rejected: Fixing the submodule check by fetching tags (`git fetch --tags`) — rejected as network-dependent and potentially heavy on a shallow submodule; the SHA assertion is deterministic and is exactly what the gitlink already guarantees. A full migration path for old persisted state — rejected as over-engineering; DSL is still at `1.0.0`, so detect-and-notify is the proportionate fix.
